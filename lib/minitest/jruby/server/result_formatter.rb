    ...
    module Server
      class ResultFormatter
        ...
        def print_run_result(result)
          ...
          @io.puts "Run #%-3d %s  %d tests, %d assertions, %d failures, %d errors in %.4fs" % [
            result[:run], status,
            result[:tests], result[:assertions], result[:failures], result[:errors],
            result[:time]
          ]
        end
        ...
        def print_summary(times:, runs:)
          ...
          @io.puts "  Mean:    %.4fs" % (times.sum / times.size)
          ...
          if times.size >= 6
            ...
            @io.puts "  Trend:   last 3 runs %.1f%% %s than first 3" % [delta.abs, direction]
          end
        end
        ...
        def format_duration(seconds)
          if seconds < 60
            ...
            "%dh %dm" % [seconds / 3600, (seconds % 3600) / 60]
          end
        end
      end
    end
